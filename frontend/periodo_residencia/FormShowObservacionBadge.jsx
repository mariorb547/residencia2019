...
export default class FormShowObservaciones extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        const { visible, tipo, id_tarea, ocultarShowObservacion,actulizarCantidadObservaciones,disabledEstadoAsesorInterno } = nextProps;
        this.setState({
            visible,
            tipo,
            id_tarea,
    ...
    onChangeObservacion = (id, estado) => {
        axios.put('/api/seguimiento/onChange_observacion', {
            id,
            estado
        }).then(res => {
            if (res.status === 200) {
                message.success('Los cambios se han guardado.')
                this.state.actulizarCantidadObservaciones()
